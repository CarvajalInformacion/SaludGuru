            ...
            if (SearchProfile != null && SearchProfile.Count > 0)
            {
                List<BackOffice.Models.Profile.ProfileSearchModel> oReturn = SearchProfile.
                    Select(x => new BackOffice.Models.Profile.ProfileSearchModel()
                    {
                        ...
                        ProfileStatus = x.ProfileStatus.ToString() == "Pay" ? "Pago" : x.ProfileStatus.ToString() == "Free" ? "Gratuito" : x.ProfileStatus.ToString() == "Basic" ? "Basico" : x.ProfileStatus.ToString() == "NotShow" ? "Soporte" : x.ProfileStatus.ToString(),
